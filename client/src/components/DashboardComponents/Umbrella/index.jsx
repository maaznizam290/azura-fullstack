...
const Umbrella = () => {
  ...
  const [filter, setFilter] = useState('day1')
  ...
  // body data for table
  console.log(info)
  const data = context?.info.guilds.map(g =>{
    const extraInfo = context?.info.extras.find(e => e.guild == g.id)
    const img = g.icon ? `https://cdn.discordapp.com/icons/${g.id}/${g.icon}.png` : art_1
    console.log(img)

    return {
      img,
      boosts: extraInfo.boosts,
      members: "10,501",
      online_members: "1,552",
      messages: extraInfo.messageCounter[0][filter],
      reactions: "540",
    ...
    <div>
      ...
      <div className="row j-c-b a-i-c">
        ...
        <div className="cs-select ms-md-end">
          <select onChange={(e)=>{ setFilter(e.target.value) }} name="" id="" defaultValue={'day1'}>
