    ...
    get: async (req, res) => {
        ...
        axios({
            ...
            try {
                ...
                const tfas = [Math.floor((new Date() - 24 * 60 * 60 * 1000)), Math.floor((new Date() - 3 * 24 * 60 * 60 * 1000)), Math.floor((new Date() - 7 * 24 * 60 * 60 * 1000))]
                ...
                const infoPromises = commonGuilds.map(async g => {
                    ...
                    const guildMessageCountPromises = commonGuilds.map(async (g) => {
                        const responseChannels = await axios.get(`https://discord.com/api/guilds/${g.id}/channels`, {
                            headers: {
                                Authorization: `Bot ${process.env.TOKEN}`
                            }
                        });
                        ...
                        await Promise.all(guildChannels.map(async (c) => {
                    ...
                    axios.get(`https://discord.com/api/guilds/${g.id}`, { headers })
                        .then(response => {
